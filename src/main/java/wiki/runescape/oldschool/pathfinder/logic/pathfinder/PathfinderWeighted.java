...
public abstract class PathfinderWeighted implements Pathfinder {

    final protected Graph graph;

    protected PathfinderWeighted(final Graph graph) {
        this.graph = graph;
    }
    ...
    public PathfinderResult findPath(Coordinate start, Coordinate end, HashSet<String> blacklist) {
        ...
        if (start.equals(end)) {
            return new PathfinderResult(
                    true,
                    List.of(new Movement(start, PathfinderResult.MOVEMENT_START_TITLE)),
                    0,
